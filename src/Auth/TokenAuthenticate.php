 ...
 * ```
 *
 * @copyright 2020 Axel Natanael Chacon Juan
 * @license MIT
 *
 * @see http://jwt.io
...
class TokenAuthenticate extends BaseAuthenticate {
    ...
    public function getUser(ServerRequest $request) {
        ...
        if (isset($user[$this->_config['fields']]) && !empty($this->_config['fields'])) {
    ...
    public function getToken($request = null) {
        ...
        $header = isset($config['header']) ? $request->getHeaderLine($config['header']) : null;
        $prefix = isset($config['prefix']) ? $config['prefix'] : "";
        if ($header && stripos($header, $prefix) === 0) {
            return $this->_token = str_ireplace($prefix . ' ', '', $header);
        }
    ...
    public function unauthenticated(ServerRequest $request, Response $response) {
        ...
        $message = $this->_error ? $this->_error->getMessage() : $this->_registry->get('Auth')->getConfig('authError');
